...
四版本共享同一 Token 硬上限审计（`registry._audit_token_budget`）。
...
| Step 2 并发与锁安全 | `37aa6bd` | ArtifactStore 内部 RLock + 元数据、agent_trace 原子追加、dispatch 工具锁隔离（Local/MCP 双路径 `UNLOCKED_TOOL_NAMES` + `should_serialize_tool()`）、store_view、349 行并发测试 |
...
| Step 4 生产切换与收敛 | **未提交（本次暂存）** | 生产入口切换 `run_production_turn`；V0–V3 收敛为同一 Engine 薄适配器；删除 M9/layered 全部代码；外围迁移（settings/server/harness/eval/scripts/web/docs）；可观测性统一 agent_trace；Renderer Gate 口径的 Prompt/权限断言/文档同步 |

## 4. 关键 Git commit 与 tag

- Step 3 commit：**`98b1a21`**（main HEAD，Step 4 的基线）
- M9 历史 tag：**`m9-layered-final`**（指向 `98b1a21`，保留删除前的完整 M9 五层代码）
- Step 2 commit：`37aa6bd`；Step 1 commit：`2b6ef64`
- Step 4 改动：工作区暂存中（尚未 commit）
...
- **锁隔离（防嵌套死锁）**：per-session 工具锁 `session.session_tool_lock` 是
  不可重入 `threading.Lock`；`dispatch_subagent` 类工具永远不得被
  `lc_tools serialized()` 或 MCP 包装加锁（否则 dispatch 持锁 → Subagent 调业务
  工具 → 同线程再取同锁死锁）。实现：`session.UNLOCKED_TOOL_NAMES` 白名单 +
  `should_serialize_tool()` 在 Local 与 MCP 两条路径统一跳过。
...
```text
pytest tests/  →  321 passed, 1 skipped
...
4. **Renderer Gate**：`render_itinerary` 是否总是绑定最新 plan_artifact_id；
   Orchestrator 工具白名单是否确实剔除渲染工具；V0/V1/V2 是否也经 Gate 渲染。
5. **锁与并发**：`UNLOCKED_TOOL_NAMES` 白名单是否被后续改动破坏；
   新增会派生子任务的工具必须同样豁免序列化锁。
